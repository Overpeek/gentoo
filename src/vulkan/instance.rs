...
impl Instance {
    pub fn new() -> anyhow::Result<Self, GentooRenderError> {
        let entry = unsafe {
            ash::Entry::load()?
        };

        let application_name = CString::new("Gentoo").unwrap();
    ...
    ) -> anyhow::Result<(ash::extensions::ext::DebugUtils, ash::vk::DebugUtilsMessengerEXT), GentooRenderError> {
    ...
    fn check_validation_layer_support(entry: &ash::Entry) -> anyhow::Result<bool, GentooRenderError> {
